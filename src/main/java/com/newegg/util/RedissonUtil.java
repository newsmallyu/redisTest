...
public class RedissonUtil {
    ...
    public static void set(String key, String value, int extimes, int retries) {
        ...
        try {
            if (extimes > 0) {
                bucket.set(value, extimes, TimeUnit.MILLISECONDS);
            } else {
                bucket.set(value);
            }
        } catch (Exception e) {
     ...
     * @return
     */
    public static boolean PFADD(String key, List<Object> valueList) {
        RHyperLogLog<Object> hyperLogLog = redissonClient.getHyperLogLog(key);
        boolean add = hyperLogLog.addAll(valueList);
        return add;
    }
     ...
     * @param sourceKey2
     */
    public static void PFMERGE(String destkey,String sourceKey1,String sourceKey2){
        RHyperLogLog<Object> hyperLogLog = redissonClient.getHyperLogLog(destkey);
        hyperLogLog.mergeWith(sourceKey1,sourceKey2);
    }
    ...
    public void testPFadd(){
        ...
        list.add("java");
        list.add("go");
        boolean pfadd = PFADD("{xaecbd}:testHyperLogLog", list);
        System.out.println(pfadd);
    }
    @Test
    public void testPFadd2(){
        List list = new ArrayList();
        list.add("c#");
        list.add("net");
        list.add("VB");
        list.add("java");
        boolean pfadd = PFADD("{xaecbd}:testHyperLogLog2", list);
        System.out.println(pfadd);
    }
    @Test
    public void testPFcount(){
        long pfcount = PFCOUNT("{xaecbd}:testHyperLogLogDest");
        System.out.println(pfcount);
    }
    @Test
    public void testPFmerge(){
        PFMERGE("{xaecbd}:testHyperLogLogDest", "{xaecbd}:testHyperLogLog", "{xaecbd}:testHyperLogLog2");
    }
